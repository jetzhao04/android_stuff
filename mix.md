# books
- [free programming books](https://github.com/vhf/free-programming-books/blob/master/free-programming-books.md)

# programming languages 
- [programming languages ranking](http://www.tiobe.com/index.php/content/paperinfo/tpci/index.html)
- [overapi](http://overapi.com/)

# regex
- [regex java](http://www.regexplanet.com/advanced/java/index.html)

# time 
- [unix time epoch converter](http://www.epochconverter.com/)

# http
- [url encode / decode](http://www.url-encode-decode.com/)

# js
- [jsfiddle](http://jsfiddle.net)
- [jsbin - collaborative js debugging](http://jsbin.com/)
- [jscoach](https://js.coach/)
- [plunker](http://plnkr.co/edit/?p=catalogue)

# python
- [python module of the week](https://pymotw.com/2/contents.html)
- [dive into python](http://www.diveintopython.net/toc/index.html)
- [automate boring stuff with python](https://automatetheboringstuff.com/chapter11/)

# commandline 
- [commandlinefu](http://www.commandlinefu.com/commands/browse/sort-by-votes)
- [The art of command line](https://github.com/jlevy/the-art-of-command-line)
- [command line tools for structure text](https://github.com/dbohdan/structured-text-tools)
- [unix tool box ](http://cb.vu/unixtoolbox.xhtml)
- [ss64](http://ss64.com/)

# bash
- [bash handbook](https://github.com/denysdovhan/bash-handbook)

# css
- [CSS getting started](https://developer.mozilla.org/en-US/docs/Web/Guide/CSS/Getting_started)

# php / hack
- [hack doc](https://docs.hhvm.com)

# git 
- [git book](http://git-scm.com/book/en/v2)

# image
- [pexels: free stock images](https://www.pexels.com/)

# design
- [FB design handskit](http://facebook.design/handskit.html)

# c# 
- [.net fiddle](http://dotnetfiddle.net/)
- [LINQ 101](http://code.msdn.microsoft.com/101-LINQ-Samples-3fb9811b)

# startup
- [Startup Stash - Curated resources and tools for startups](http://startupstash.com/)
- [builtwith](Startup Stash - Curated resources and tools for startups)
